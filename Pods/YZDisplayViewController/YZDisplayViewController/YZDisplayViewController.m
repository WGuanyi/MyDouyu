...
- (void)setUpTitleWidth
{
    ...
    for (NSString *title in titles) {
        ...
        CGFloat width = 0;
#warning 修改的地方，customLabelsWidth
        if (self.customLabelsWidth) {
            width = self.customLabelsWidth.floatValue;
        } else {
            ...
            width = titleBounds.size.width;
        }
        [self.titleWidths addObject:@(width)];
        totalWidth += width;
    }
#warning 修改的地方，customMargin
    if (totalWidth >= YZScreenW) {
        // 判断是否自定义了间隙
        if (self.customMargin) {
            _titleMargin = self.customMargin.floatValue;
        } else {
            _titleMargin = margin;
        }
