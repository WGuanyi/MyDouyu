...
//--------------------------------------------
@interface LiveTopButton : UIButton


@end

@implementation LiveTopButton
...
@interface LiveTopView () <LiveDownViewDelegate>
 ...
 *  顶部的选中按钮
 */
@property (nonatomic,strong) LiveTopButton *lastBtn;
/**
 *  下拉出现的view
 */
...
- (void)setup {
    ...
    self.layer.borderColor = MyColor(233, 233, 233).CGColor;
...
- (void)btnClick:(UIButton *)btn {
    if (btn.selected) {
        ...
        // 显示下拉的列表
        [UIView animateWithDuration:0.3 animations:^{
            self.downView.frame = CGRectMake(0, 50, self.superView.width, 450);
        }];
    }
}
...
- (void)setTags:(NSArray *)tags {
    ...
    LiveTopButton *allBtn = [[LiveTopButton alloc] init];
    [allBtn addTarget:self action:@selector(topButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    [allBtn setTitle:@"全部" forState:UIControlStateNormal];
    [allBtn setBackgroundImage:[self buttonImageFromColor:MyColor(238, 238, 238)] forState:UIControlStateNormal];
    ...
    for (int i = 0; i < tags.count; i++) {
        MyTag *tag = tags[i];
        LiveTopButton *btn = [[LiveTopButton alloc] init];
        [btn addTarget:self action:@selector(topButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        [btn setBackgroundImage:[self buttonImageFromColor:MyColor(238, 238, 238)] forState:UIControlStateNormal];
 ...
 *  点击了顶部的按钮
 */
- (void)topButtonClicked:(LiveTopButton *)btn {
    if (!btn.selected) {
        btn.selected = YES;
        ...
        if ([[btn titleForState:UIControlStateNormal] isEqualToString:@"全部"]) {
            if ([self.topViewDelegate respondsToSelector:@selector(liveTopView:didClickButtonTitle:)]) {
                [self.topViewDelegate liveTopView:self didClickButtonTitle:@"全部"];
            }
        } else if ([self.topViewDelegate respondsToSelector:@selector(liveTopView:didClickButtonTagNumber:)]) {
            [self.topViewDelegate liveTopView:self didClickButtonTagNumber:btn.tag];
        }
...
- (void)setupBtnsFrame {
    ...
    CGFloat btnW = 0;
    CGFloat btnX = 0;
    LiveTopButton *lastBtn = nil;
    for (int i = 0; i < self.topBtns.count; i++) {
        LiveTopButton *liveBtn = self.topBtns[i];
        if (i == 0) {
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    ...
    LiveDownView *downView = [[LiveDownView alloc] init];
    // 设置代理
    downView.downViewDelegate = self;
    downView.frame = CGRectMake(0, 50, self.superView.width, 0);
    downView.backgroundColor = MyColor(221, 221, 220);
...
- (void)liveDownView:(LiveDownView *)downView DidClickDownButtonViewName:(NSString *)name {
    for (LiveTopButton *btn in self.topBtns) {
