...
@interface LivePlayViewController () <PortraitViewDelegate, LandScapeViewDelegate, SettingScrollViewDelegate, RoomListViewDelegate>
 ...
 *  频道选择的view
 */
@property (nonatomic,strong) ChannelSelectScrollView *channelScrollView;

/**
 *  设置的view
 */
@property (nonatomic,strong) SettingScrollView *settingScrollView;

/**
 *  展示房间列表的view
 */
@property (nonatomic,strong) RoomListView *roomListView;
...
+ (instancetype)sharedLivePlayViewController{
    static LivePlayViewController *_instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[LivePlayViewController alloc] init];
    });
    return _instance;
}
 ...
 *  频道选择的view
 */
- (ChannelSelectScrollView *)channelScrollView {
    if (!_channelScrollView) {
        _channelScrollView = [[ChannelSelectScrollView alloc] init];
        _channelScrollView.channels = @[@"主线路", @"备用线路1", @"备用线路2", @"备用线路3"];
    }
...
- (SettingScrollView *)settingScrollView {
    if (!_settingScrollView) {
        _settingScrollView = [SettingScrollView settingScrollView];
    }
...
- (void)commonCoverTaped:(UITapGestureRecognizer *)tapGest {
    ...
    [self.channelScrollView mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (void)setup {
    ...
    // 添加一个竖屏下控制的View
    [self.blackCoverView addSubview:self.portraitView];
    // 设置竖屏控制view的约束
    ...
    // 添加一个横屏下控制的view
    [self.blackCoverView addSubview:self.landScapeView];
    self.landScapeView.hidden = YES;
    
    // 添加一个controlCoverView，来控制控制view显示和隐藏
    UIView *controlCoverView = [[UIView alloc] init];
    [self.blackCoverView insertSubview:controlCoverView belowSubview:self.portraitView];
    ...
    __weak LivePlayViewController *weakSelf = self;
    self.channelScrollView.selectedChannel = ^(NSString *title, NSString *channelString) {
    ...
    [self.blackCoverView insertSubview:_renderer.view belowSubview:self.landScapeView];
...
- (void)setPlayOrPause {
    ...
    if (self.moviePlayer.isPlaying) {
        ...
        [self.portraitView setIsPlaying:NO];
        [self.landScapeView setIsPlaying:NO];
        
    } else {
        [self.moviePlayer play];
        [self.portraitView setIsPlaying:YES];
        [self.landScapeView setIsPlaying:YES];
    }
}
/**
 *  设置的方向改变了
...
- (void)setOrientationLandscape {
    ...
    self.lockBtn.hidden = YES;
    // 设置横屏的控制约束
    [self.landScapeView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.blackCoverView.mas_top);
        make.left.equalTo(self.blackCoverView.mas_left);
    ...
    [self addSpriteTimer];
    // 设置横屏的弹幕为打开模式
    [self.landScapeView setIsOpenBarrage:YES];
}
...
#pragma mark - PortraitControlViewDelegate
/**
 *  返回
 */
...
#pragma mark - LandScapeControlViewDelegate
/**
 *  退出全屏
 */
 ...
 *  打开或关闭礼物
 */
- (void)landScapeView:(LandScapeView *)landScapeView didCliciGiftBtn:(BOOL)isOpenGift {
...
- (void)landScapeViewDidClickChannelSelectBtn:(LandScapeView *)landScapeView {
    ...
    self.channelScrollView.hidden = NO;
    
    [self deleteTimer];
    [[UIApplication sharedApplication] setStatusBarHidden:YES withAnimation:UIStatusBarAnimationFade];
    self.landScapeView.hidden = YES;
    
    [self.view insertSubview:self.commonCoverView belowSubview:self.channelScrollView];
...
#pragma mark - SettingScrollViewDelegate
/**
 *  修改了弹幕的属性
 */
